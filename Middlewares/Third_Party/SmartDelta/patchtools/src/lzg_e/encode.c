...
*
* Modified by e2 for FOTA - removed unneded features
*
...
/* Limits */
//e2#define _LZG_MAX_RUN_LENGTH 128
#define _LZG_MAX_RUN_LENGTH (sizeof(_LZG_LENGTH_QUANT_LUT) / sizeof(*_LZG_LENGTH_QUANT_LUT) - 1)
...
static void _LZG_SetHeader(unsigned char *out, lzg_header *hdr)
{
    ...
    /* Checksum */
    hdr->checksum = 0;//_LZG_CalcChecksum(&out[LZG_HEADER_SIZE], hdr->encodedSize);
...
{
//e2    return LZG_HEADER_SIZE + insize;
    return insize;
}
...
void LZG_InitEncoderConfig(lzg_encoder_config_t *config)
{
    ...
    //e2 config->level = LZG_LEVEL_DEFAULT;
    config->window_order = 15;
...
lzg_int32_t LZG_EncodeFull(const unsigned char *in, lzg_uint32_t insize,
...
//e2    lzg_header hdr;
    ...
    /* Check arguments */
//e2    if ((!in) || (!out) || (outsize < (LZG_HEADER_SIZE + insize)))
    ...
    params = &_LZG_TUNING_PARAMETERS[level - 1];
#else
    static tune_params_t tp;
    tp.window = 1 << config->window_order;
    tp.maxMatches = tp.window;
    tp.goodLength = _LZG_MAX_RUN_LENGTH;
    params = &tp;
#endif
...
//e2    return LZG_HEADER_SIZE + hdr.encodedSize;
    return (dst - out);
...
lzg_int32_t LZG_Encode(const unsigned char *in, lzg_uint32_t insize,
    unsigned char *out, lzg_uint32_t outsize, lzg_encoder_config_t *config)
{
